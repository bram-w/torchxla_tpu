...
class CLIP(nn.Module):
    ...
    def forward(self, image, text, xla_gather=True):
        ...
        print(f"Globals", torch.any(torch.isnan( global_image_features)),
                torch.any(torch.isnan(global_text_features)))
        ...
        print(f"Logits", torch.any(torch.isnan(logits_per_image)),
...
class CLIP_LITE_REPLICA(nn.Module):
    def __init__(self):
        super().__init__()
...
def build_model(state_dict: dict, load_pretrained_weights=True):
